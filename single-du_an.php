<?php
/**
 * Template for displaying single Dự án post
 *
...
while (have_posts()) : the_post();
    ...
    if ($project_terms && !is_wp_error($project_terms)) {
        ...
        $category_id = $project_terms[0]->term_id;
    }
    
    // Get company name from meta
    $company_name = get_post_meta($post_id, '_quantrieu_ten_cong_ty', true);
    
    // Get related projects (from same category, excluding current post)
    $related_args = array(
        'post_type' => 'du_an',
        'posts_per_page' => 5,
...
<article class="pt-24 pb-12 bg-background" data-animate="fade">
    <div class="container mx-auto px-4">
        ...
        <nav class="flex flex-wrap items-center gap-2 text-sm text-muted-foreground mb-8 bg-muted/30 py-3 px-4 rounded-lg">
            ...
            <a class="hover:text-[#FF9800] transition-colors whitespace-nowrap flex-shrink-0" href="<?php echo esc_url(get_post_type_archive_link('du_an')); ?>">
                Dự án
            </a>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-right w-4 h-4 flex-shrink-0">
                <path d="m9 18 6-6-6-6"></path>
            ...
            <aside class="w-full lg:w-[300px] flex-shrink-0" data-animate="fade-left">
                <div class="lg:sticky lg:top-24 space-y-6">
                    ...
                    <?php if ($related_projects->have_posts()) : ?>
                        <div class="bg-muted/50 rounded-lg p-5">
                            <h3 class="font-semibold text-foreground mb-4">Dự án khác</h3>
                            <ul class="space-y-3">
                            ...
                            <a class="text-sm text-[#0090ff] hover:underline mt-4 inline-block" 
                               href="<?php echo esc_url(get_post_type_archive_link('du_an')); ?>">
                                Xem tất cả dự án
                            </a>
                        </div>
                    <?php endif; ?>
...
<section class="py-20 bg-muted/50" data-animate="fade-up">
    <div class="container mx-auto px-4">
        <div class="text-center mb-12">
            ...
            <h2 class="text-3xl md:text-4xl font-bold mb-4">Dự Án Liên Quan</h2>
            <p class="text-muted-foreground max-w-2xl mx-auto">
                Khám phá thêm các dự án in ấn tiêu biểu khác của In Quan Triều
            </p>
        </div>
        
        <?php
        // Get more related projects for the section (4 posts)
        ...
        ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <?php while ($section_related->have_posts()) : $section_related->the_post(); ?>
                    ...
                    $related_company = get_post_meta(get_the_ID(), '_quantrieu_ten_cong_ty', true);
                    ?>
                       ...
                       href="<?php the_permalink(); ?>">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-5">
                            <?php if ($related_company) : ?>
                                <span class="inline-block px-2 py-0.5 bg-[#FF9800] text-background text-xs font-medium rounded mb-2">
                                    <?php echo esc_html($related_company); ?>
                                </span>
                            <?php endif; ?>
                            ...
                            <p class="text-background/70 text-sm line-clamp-2">
                                <?php echo $related_excerpt ? esc_html($related_excerpt) : 'Xem chi tiết...'; ?>
                            </p>
                        </div>
